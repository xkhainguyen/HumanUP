...
class JoystickController:
    def __init__(self, config: Config) -> None:
        ...
        self.policy = torch.jit.load("deploy/pre_train/g1/motion.pt")
    ...
    def run(self):
        ...
        self.obs[9 + num_actions * 3] = sin_phase*1                               # phase
        self.obs[9 + num_actions * 3 + 1] = cos_phase*1                           # phase
...
class Controller:
    def __init__(self, config: Config) -> None:
        ...
        self.n_priv = 0
        self.n_proprio = 3 + 2 + 2*self.config.num_actions + self.config.num_actions
    ...
    def run(self):
        ...
        qj_obs = self.qj.copy()
        dqj_obs = self.dqj.copy()
        qj_obs = (qj_obs - self.config.default_23_angles) * self.config.dof_pos_scale
        dqj_obs = dqj_obs * self.config.dof_vel_scale
        ang_vel = ang_vel * self.config.ang_vel_scale
        ...
        all_dof_actions[self.config.action_joint2motor_idx] = self.config.default_23_angles + raw_action * self.config.action_scale
        ...
        for i in range(len(self.config.arm_waist_joint2motor_idx)):
            ...
            self.low_cmd.motor_cmd[motor_idx].qd = 0
            self.low_cmd.motor_cmd[motor_idx].kp = self.config.arm_waist_kps[i] * 1.15
...
if __name__ == "__main__":
    ...
    config.default_23_angles = np.array([-0.1,  0.0,  0.0,  0.3, -0.2, 0.0,
